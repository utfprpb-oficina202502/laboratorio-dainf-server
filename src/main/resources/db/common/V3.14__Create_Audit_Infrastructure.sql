...
--
-- ESTRUTURA:
--   revinfo: Tabela de revisoes (metadados de cada operacao)
--   *_aud: Tabelas de historico para cada entidade auditada
--
-- CAMPOS PADRAO NAS TABELAS _aud:
--   id: ID da entidade original (compoe PK com rev)
--   rev: FK para revinfo (numero da revisao)
--   revtype: Tipo de operacao (0=INSERT, 1=UPDATE, 2=DELETE)
--   *_mod: Indica se o campo foi modificado nesta revisao
...
COMMENT ON TABLE revinfo IS 'Tabela de revisoes do Hibernate Envers - armazena metadados de cada operacao auditada';
COMMENT ON COLUMN revinfo.id IS 'Identificador unico da revisao';
COMMENT ON COLUMN revinfo.timestamp IS 'Timestamp Unix da revisao (millisegundos desde epoch)';
COMMENT ON COLUMN revinfo.usuario IS 'Username do usuario que executou a operacao';
COMMENT ON COLUMN revinfo.ip IS 'Endereco IP da requisicao (suporta IPv4 e IPv6)';
