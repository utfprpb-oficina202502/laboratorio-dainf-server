...
class EmprestimoServiceImplTest {
  ...
  @Test
  void testProcessDevolucao_TodosItensPendentes_FinalizaEmprestimo() {
    // Given - Devolução completa, todos itens P
    Emprestimo emprestimoTeste = new Emprestimo();
    emprestimoTeste.setId(1L);
    emprestimoTeste.setDataDevolucao(null);

    Item item = new Item();
    item.setId(100L);

    EmprestimoDevolucaoItem itemDevolucao = new EmprestimoDevolucaoItem();
    itemDevolucao.setId(1L);
    itemDevolucao.setItem(item);
    itemDevolucao.setQtde(BigDecimal.TEN);
    itemDevolucao.setStatusDevolucao(StatusDevolucao.P);

    emprestimoTeste.setEmprestimoDevolucaoItem(new ArrayList<>(List.of(itemDevolucao)));
    emprestimoTeste.setUsuarioEmprestimo(usuarioEmprestimo);

    // Mock para super.save retornar empréstimo salvo
    Emprestimo emprestimoSalvo = new Emprestimo();
    emprestimoSalvo.setId(1L);
    emprestimoSalvo.setDataDevolucao(null);
    emprestimoSalvo.setUsuarioEmprestimo(usuarioEmprestimo);

    EmprestimoDevolucaoItem itemSalvo = new EmprestimoDevolucaoItem();
    itemSalvo.setId(1L);
    itemSalvo.setItem(item);
    itemSalvo.setQtde(BigDecimal.TEN);
    itemSalvo.setStatusDevolucao(StatusDevolucao.D); // Mudou para D após processamento

    emprestimoSalvo.setEmprestimoDevolucaoItem(new ArrayList<>(List.of(itemSalvo)));

    EmprestimoResponseDto emprestimoSalvoDto = new EmprestimoResponseDto();
    emprestimoSalvoDto.setId(1L);

    doReturn(emprestimoSalvoDto).when(service).save(any(Emprestimo.class));
    when(modelMapper.map(any(EmprestimoResponseDto.class), eq(Emprestimo.class)))
        .thenReturn(emprestimoSalvo);
    when(modelMapper.map(any(Emprestimo.class), eq(EmprestimoResponseDto.class)))
        .thenReturn(emprestimoSalvoDto);
    ...
    assertNotNull(result);
    verify(service).save(any(Emprestimo.class));
    verify(itemService).aumentaSaldoItem(100L, BigDecimal.TEN);
    verify(eventPublisher).publishEvent(any()); // Email de confirmação
  }

  @Test
  void testProcessDevolucao_ItensMistos_NaoFinalizaEmprestimo() {
    // Given - Alguns itens ainda pendentes (D + P)
    Emprestimo emprestimoTeste = new Emprestimo();
    emprestimoTeste.setId(1L);
    emprestimoTeste.setDataDevolucao(null);
    ...
    EmprestimoDevolucaoItem itemDevolvido = new EmprestimoDevolucaoItem();
    itemDevolvido.setItem(item1);
    itemDevolvido.setQtde(BigDecimal.valueOf(5));
    itemDevolvido.setStatusDevolucao(StatusDevolucao.D);

    EmprestimoDevolucaoItem itemPendente = new EmprestimoDevolucaoItem();
    itemPendente.setItem(item2);
    itemPendente.setQtde(BigDecimal.valueOf(3));
    itemPendente.setStatusDevolucao(StatusDevolucao.P);

    emprestimoTeste.setEmprestimoDevolucaoItem(
        new ArrayList<>(List.of(itemDevolvido, itemPendente)));
    emprestimoTeste.setUsuarioEmprestimo(usuarioEmprestimo);

    // Mock
    Emprestimo emprestimoSalvo = new Emprestimo();
    emprestimoSalvo.setId(1L);
    emprestimoSalvo.setDataDevolucao(null); // Deve continuar null
    emprestimoSalvo.setUsuarioEmprestimo(usuarioEmprestimo);
    emprestimoSalvo.setEmprestimoDevolucaoItem(
        new ArrayList<>(List.of(itemDevolvido, itemPendente)));
    ...
    // Then
    assertNotNull(result);
    verify(itemService).aumentaSaldoItem(100L, BigDecimal.valueOf(5)); // Apenas o devolvido
    verify(itemService, never()).aumentaSaldoItem(eq(200L), any()); // Pendente não aumenta saldo
  }

  @Test
  void testProcessDevolucao_ItensParaSaida_CriaSaida() {
    // Given - Itens marcados para saída (S)
    Emprestimo emprestimoTeste = new Emprestimo();
    emprestimoTeste.setId(1L);
    emprestimoTeste.setUsuarioEmprestimo(usuarioEmprestimo);
    ...
    EmprestimoDevolucaoItem itemSaida = new EmprestimoDevolucaoItem();
    itemSaida.setItem(item);
    itemSaida.setQtde(BigDecimal.valueOf(5));
    itemSaida.setStatusDevolucao(StatusDevolucao.S);

    emprestimoTeste.setEmprestimoDevolucaoItem(new ArrayList<>(List.of(itemSaida)));

    // Mock
    Emprestimo emprestimoSalvo = new Emprestimo();
    emprestimoSalvo.setId(1L);
    emprestimoSalvo.setUsuarioEmprestimo(usuarioEmprestimo);
    emprestimoSalvo.setEmprestimoDevolucaoItem(new ArrayList<>(List.of(itemSaida)));
    ...
    // Then
    verify(saidaService).createSaidaByDevolucaoEmprestimo(anyList());
    verify(itemService, never()).aumentaSaldoItem(any(), any()); // S não aumenta saldo
  }

  @Test
  void testProcessDevolucao_ListaDevolucaoNull_NaoLancaExcecao() {
    // Given - Empréstimo sem itens de devolução
    Emprestimo emprestimoTeste = new Emprestimo();
    emprestimoTeste.setId(1L);
    emprestimoTeste.setEmprestimoDevolucaoItem(null);
    emprestimoTeste.setUsuarioEmprestimo(usuarioEmprestimo);

    Emprestimo emprestimoSalvo = new Emprestimo();
    emprestimoSalvo.setId(1L);
    emprestimoSalvo.setEmprestimoDevolucaoItem(null);
    emprestimoSalvo.setUsuarioEmprestimo(usuarioEmprestimo);
    ...
    verify(saidaService, never()).createSaidaByDevolucaoEmprestimo(any());
  }

  @Test
  void testProcessDevolucao_MistoComTodosStatus_ProcessaCorretamente() {
    // Given - Mix completo: D, S e P
    Emprestimo emprestimoTeste = new Emprestimo();
    emprestimoTeste.setId(1L);
    emprestimoTeste.setUsuarioEmprestimo(usuarioEmprestimo);
    ...
    EmprestimoDevolucaoItem itemD = new EmprestimoDevolucaoItem();
    itemD.setItem(item1);
    itemD.setQtde(BigDecimal.valueOf(5));
    itemD.setStatusDevolucao(StatusDevolucao.D);

    EmprestimoDevolucaoItem itemS = new EmprestimoDevolucaoItem();
    itemS.setItem(item2);
    itemS.setQtde(BigDecimal.valueOf(3));
    itemS.setStatusDevolucao(StatusDevolucao.S);

    EmprestimoDevolucaoItem itemP = new EmprestimoDevolucaoItem();
    itemP.setItem(item3);
    itemP.setQtde(BigDecimal.valueOf(2));
    itemP.setStatusDevolucao(StatusDevolucao.P);

    emprestimoTeste.setEmprestimoDevolucaoItem(new ArrayList<>(List.of(itemD, itemS, itemP)));

    // Mock
    Emprestimo emprestimoSalvo = new Emprestimo();
    emprestimoSalvo.setId(1L);
    emprestimoSalvo.setUsuarioEmprestimo(usuarioEmprestimo);
    emprestimoSalvo.setEmprestimoDevolucaoItem(new ArrayList<>(List.of(itemD, itemS, itemP)));
    ...
    // Then
    verify(itemService).aumentaSaldoItem(100L, BigDecimal.valueOf(5)); // Apenas D
    verify(itemService, never()).aumentaSaldoItem(eq(200L), any()); // S vai para saída
    verify(itemService, never()).aumentaSaldoItem(eq(300L), any()); // P ainda pendente
    verify(saidaService).createSaidaByDevolucaoEmprestimo(anyList()); // S cria saída
  }
}
